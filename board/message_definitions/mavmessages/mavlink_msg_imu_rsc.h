...
#define MAVLINK_MESSAGE_INFO_IMU_rsc { \
    ...
    {  { "accel", NULL, MAVLINK_TYPE_FLOAT, 3, 0, offsetof(mavlink_imu_rsc_t, accel) }, \
         ...
         { "magn", NULL, MAVLINK_TYPE_FLOAT, 3, 24, offsetof(mavlink_imu_rsc_t, magn) }, \
...
static inline uint16_t mavlink_msg_imu_rsc_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               const float *accel, const float *gyro, const float *magn, float time)
{
    ...
    _mav_put_float_array(buf, 24, magn, 3);
...
{
    return mavlink_msg_imu_rsc_pack(system_id, component_id, msg, imu_rsc->accel, imu_rsc->gyro, imu_rsc->magn, imu_rsc->time);
}
...
{
    return mavlink_msg_imu_rsc_pack_chan(system_id, component_id, chan, msg, imu_rsc->accel, imu_rsc->gyro, imu_rsc->magn, imu_rsc->time);
}
...
static inline void mavlink_msg_imu_rsc_send(mavlink_channel_t chan, const float *accel, const float *gyro, const float *magn, float time)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_IMU_rsc_LEN];
    _mav_put_float(buf, 36, time);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_imu_rsc_send(chan, imu_rsc->accel, imu_rsc->gyro, imu_rsc->magn, imu_rsc->time);
#else
 ...
 */
static inline void mavlink_msg_imu_rsc_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  const float *accel, const float *gyro, const float *magn, float time)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field magn from imu_rsc message
 *
 * @return  compass (magnetometer) vector
 */
static inline uint16_t mavlink_msg_imu_rsc_get_magn(const mavlink_message_t* msg, float *magn)
{
    return _MAV_RETURN_float_array(msg, magn, 3,  24);
}
...
static inline void mavlink_msg_imu_rsc_decode(const mavlink_message_t* msg, mavlink_imu_rsc_t* imu_rsc)
{
    ...
    mavlink_msg_imu_rsc_get_magn(msg, imu_rsc->magn);
