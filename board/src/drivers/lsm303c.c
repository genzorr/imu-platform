...
static int lsm303c_m_do_read_regn_i2c(const struct lsm303c_handler_s * priv, uint8_t regaddr, int8_t * data, size_t datasize)
{
	int rc = HAL_I2C_Mem_Read(priv->setup_conf.iface.i2c.hi2c ,priv->setup_conf.iface.i2c.addr_m, regaddr, I2C_MEMADD_SIZE_8BIT, (uint8_t*)data, datasize, LSM303C_TIMEOUT);
	if(rc)
...
static int lsm303c_m_do_write_regn_i2c( struct lsm303c_handler_s * priv, uint8_t regaddr, uint8_t * data, size_t datasize)
{
